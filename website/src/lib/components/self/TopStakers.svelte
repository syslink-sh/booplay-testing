<script lang="ts">
	...
	let stakersColumns = $derived([
		{ key: 'rank', label: 'Rank', class: 'w-[10%] min-w-[60px]', render: (value: any) => `#${value}` },
		{
			key: 'user', label: 'User', class: 'w-[35%] min-w-[150px]',
			render: (value: any, row: any) => ({ component: 'user', image: row.image, name: row.name || 'Anonymous', username: row.username })
		},
		{ key: 'quantity', label: 'Staked', class: 'w-[20%] min-w-[100px] font-mono', sortable: false, render: (value: any) => formatQuantity(value) },
		{
			key: 'percentage', label: '% of Pool', class: 'w-[12%] min-w-[70px]', sortable: false,
			render: (value: any) => ({ component: 'badge', variant: 'secondary', text: `${value.toFixed(1)}%` })
		},
		{ key: 'value', label: 'Est. Value', class: 'w-[23%] min-w-[90px] font-mono font-medium', sortable: true, defaultSort: 'desc' as const, render: (value: any) => formatValue(value) }
	]);
</script>

<Card.Root
	class="gap-2 {stakersData && stakersData.stakers.length > 3 ? 'hover:bg-card/90 cursor-pointer transition-colors' : ''}"
...
>
	...
	<Card.Content class="relative">
			...
			<div class="space-y-3">
				{#each stakersData.stakers.slice(0, 3) as staker}
					<div class="flex items-center gap-2">
						<div class="flex min-w-0 flex-1 cursor-pointer items-center gap-3 border-none bg-transparent p-0 text-left">
							<Avatar.Root class="h-8 w-8 flex-shrink-0">
								<Avatar.Image src={getPublicUrl(staker.image)} alt={staker.name || staker.username} />
								<Avatar.Fallback class="text-xs">{(staker.name || staker.username).charAt(0)}</Avatar.Fallback>
							</Avatar.Root>
						...
						<div class="flex flex-shrink-0 items-center gap-1.5 text-right">
							...
							<div class="flex flex-col items-end gap-0.5">
								<p class="text-muted-foreground font-mono text-xs">{formatQuantity(staker.quantity)}</p>
								<p class="text-muted-foreground/80 font-mono text-xs font-medium">{formatValue(staker.value)}</p>
							</div>
						</div>
					</div>
				{/each}
		...
		{#if stakersData && stakersData.stakers.length > 3}
			<div class="from-card/80 pointer-events-none absolute inset-0 bg-gradient-to-t via-transparent to-transparent"></div>
		{/if}
	</Card.Content>
</Card.Root>

<Dialog.Root bind:open={modalOpen}>
	<Dialog.Content class="flex max-h-[90vh] w-full max-w-[calc(100%-2rem)] flex-col sm:max-w-[800px] md:max-w-2xl">
		...
		<div class="min-h-0 flex-1">
			...
			{:else}
				<div class="flex h-full items-center justify-center py-12 text-center">
					<div>
						<HugeiconsIcon icon={MoneyBag02Icon} class="text-muted-foreground mx-auto mb-4 h-12 w-12" />
